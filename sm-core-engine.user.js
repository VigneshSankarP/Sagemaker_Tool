// ==UserScript==
// @name        SM - Core Engine (Timer & Counter)
// @namespace   https://github.com/VigneshSankarP/Sagemaker_Tool
// @version     1.0.0
// @description Core engine: parsing, tracking, sessions, commit/discard, midnight reset, storage, cross-tab, exposes window.SM_API.
// @match       https://*.console.aws.amazon.com/*
// @match       https://*.amazonaws.com/*
// @grant       none
// @updateURL   https://raw.githubusercontent.com/VigneshSankarP/Sagemaker_Tool/main/sm-core-engine.user.js
// @downloadURL https://raw.githubusercontent.com/VigneshSankarP/Sagemaker_Tool/main/sm-core-engine.user.js
// ==/UserScript==
...
(function () {
  ...
  function trackOnce() {
    ...
    if (typeof awsData.limit === 'number' && awsData.current >= awsData.limit) discardActiveTask('expired');
  ...
  function displayVisible(visible) {
    try {
      if (typeof window.SM_UI_setVisible === 'function') { window.SM_UI_setVisible(visible); return; }
      const el = document.getElementById('sm-utilization');
  ...
  window.addEventListener('storage', (e) => {
    ...
    if ([KEYS.DAILY_COMMITTED, KEYS.COUNT, KEYS.HISTORY, KEYS.LAST_DATE, KEYS.SESSIONS].includes(e.key)) {
      ...
      updateDisplayForUIs();
      const root = document.getElementById('sm-dashboard-root'); if (root && root.style.display === 'flex') {
        try { if (typeof window.SM_UI_updateDashboard === 'function') window.SM_UI_updateDashboard(); } catch(e){}
      }
    }
  });

  // --- expose stable API for UIs
  window.SM_API = {
    getData() {
      const committed = retrieve(KEYS.DAILY_COMMITTED, 0) || 0;
      ...
      const sessions = retrieve(KEYS.SESSIONS, []) || [];
      const history = retrieve(KEYS.HISTORY, {}) || {};
      const running = !!(activeTask && activeTask.status === 'active');
      const pending = activeTask ? (activeTask.awsCurrent || 0) : 0;
      return { committed, count, sessions, history, running, pending };
    },
    ...
    _internal_getActiveTask() { return activeTask; } // debug helper
  };
  ...
  window.addEventListener('keydown', (e) => { if (e.ctrlKey && e.shiftKey && e.key.toLowerCase() === 'u') {
    try { if (typeof window.SM_UI_showDashboard === 'function') window.SM_UI_showDashboard(); } catch(e){} }});
  log('SM core ready');
